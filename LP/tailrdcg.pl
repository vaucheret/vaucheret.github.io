...
dcg -->
    ...
    slide(col("red","Definite Clause Grammars"),
	  (
	      p(("Las ",col("red",em("Definite Clause Grammars (DCG)")), " son una extensión de Prolog que permite describir gramáticas formales de manera  más natural y legible. Las DCG se utilizan comúnmente para el análisis sintáctico de lenguajes formales, como los  lenguajes de programación y los lenguajes naturales. Las DCG se basan en la idea de que una gramática puede ser representada como un conjunto de cláusulas definidas en Prolog, donde cada cláusula representa una regla de producción de la gramática."))
	  )),
